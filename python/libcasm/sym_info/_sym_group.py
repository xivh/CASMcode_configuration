...
class SymGroup(_sym_info.SymGroup):
    r"""Data structure holding group elements and other group info, such as \
    ...
    multiplication_table : list[list[int]]
        ...
        indicates that ``element[k] == element[i] * element[j]``.
    ...
    def conjugacy_classes(self) -> list[list[int]]:
        ...
        conjugacy_classes : list[list[int]]
            The value ``conjugacy_classes[i][j]`` is the index of the j-th element in
            conjugacy class i.
    ...
    def head_group_index(self) -> list[int]:
        ...
        head_group_index: list[int]
            Specifies the index into the head group elements list corresponding to each
    ...
    def inv(self, i: int) -> int:
        ...
        -------
        k: int
            The index in this group's elements list of the inverse of the i-th element
            in this group.
        """
        return super().inv(i)
    ...
    def make_subgroup(
        ...
        element : Optional[list[~libcasm.xtal.SymOp]]
            List of elements in the subgroup. This is optional, and if not provided the
            head group elements corresponding to the head_group_index will be used. It
            may be provided to specify particular elements for a subgroup of a factor
            group, such as the elements of a cluster group. Must be listed in ascending
            head group index order.
        """
        return super().is_subgroup()
    ...
    def mult(self, i: int, j: int) -> int:
        ...
        -------
        k: int
            The element product index form the group multiplication table, where
            ``k == multiplication_table[i][j]`` indicates that
    ...
    def multiplication_table(self) -> list[list[int]]:
        ...
        multiplication_table: list[list[int]]
            ...
            indicates that ``element[k] == element[i] * element[j]``.
        """
        return super().multiplication_table()
