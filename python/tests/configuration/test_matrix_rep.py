...
def test_conventional_fcc_matrix_rep(FCC_binary_GLstrain_disp_prim):
    ...
    occ_rep = config.make_local_dof_matrix_rep(invariant_subgroup, "occ",
                                               site_indices)
    #print(occ_rep)
    assert len(occ_rep) == 4 * 48

    disp_rep = config.make_local_dof_matrix_rep(invariant_subgroup, "disp",
                                                site_indices)
    #print(disp_rep)
    assert len(disp_rep) == 4 * 48

    GLstrain_rep = config.make_global_dof_matrix_rep(invariant_subgroup,
                                                     "GLstrain")
    #print(GLstrain_rep)
