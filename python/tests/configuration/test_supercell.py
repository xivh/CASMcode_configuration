...
def test_simple_cubic_binary_supercell(simple_cubic_binary_prim):
    ...
    assert np.allclose(supercell.coordinate_frac(),
                       np.array([[0., 0., 0.], [1., 0., 0.]]).transpose())
    assert np.allclose(supercell.coordinate_cart(),
                       np.array([[0., 0., 0.], [1., 0., 0.]]).transpose())
    assert supercell.sublattice_indices() == [0, 0]
    assert (supercell.unitcell_indices() == np.array([[0, 0, 0],
                                                      [1, 0,
                                                       0]]).transpose()).all()
...
def test_simple_cubic_binary_supercell_compare(simple_cubic_binary_prim):
    ...
    assert np.allclose(
        canonical_supercell.superlattice().column_vector_matrix(),
        expected_S_canonical)
